...
select ulke, cinsiyet, count(isim) as calisan_sayi, round(avg(yas)) as yas_ort from insan group by ulke,cinsiyet;
...
select ulke, cinsiyet, count(isim) as calisan_sayi, round(avg(yas)) as yas_ort from insan 
where maas>30000
